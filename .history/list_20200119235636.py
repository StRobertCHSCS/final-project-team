...
def apple():
    ...
    if apple_display is True:
        ...
        apple_x = random.randint(0, COLUMN_COUNT)
        apple_y = random.randint(0, ROW_COUNT)
        for x_apple in range (len(snake_pos)):
            for y_apple in range (len(snake_pos)):
                if apple_x == snake_pos[i][0] or apple_y == snake_pos[i][1]:
                    apple_x = random.randint(0, COLUMN_COUNT)
                    apple_y = random.randint(0, ROW_COUNT)
                    break
        apple_x_coordinate = (MARGIN + WIDTH) * apple_x + MARGIN + WIDTH // 2  
...
def setup():
    ...
    SPEED = int(input("What level do you want"))
