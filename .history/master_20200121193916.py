...
# Starting screen 
alive_button = []
start_button_text = ["Noob: 0.5 speed", "Normal speed: 1", "Hard: 1.5 speed", "Expert: 2.5 speed"]

for i in range (2, 10, 2):
        start_button = [i*100, 200, 150, 50, start_button_text[(i // 2) - 1]]  # x, y, width, height
        alive_button.append(start_button)
show_text = False

# Death screen
dead_button = []
for i in range (4, 8, 4):
    death_button = [i*100, 200, 150, 50, button_text[(i // 2) - 1]]  # x, y, width, height
    dead_button.append(death_button)
...
def start_screen():
    ...
    for i in range (0, 4):
        ...
        arcade.draw_text(alive_button[i][4], alive_button[i][0] + (alive_button[i][2] // 2), alive_button[i][1] + (buttons[i][3] // 2),
...
def death_screen():
    arcade.draw_text("You died", 2*(SCREEN_WIDTH//5), 3*(SCREEN_HEIGHT//4), arcade.color.WHITE, 25, font_name='calibri')
