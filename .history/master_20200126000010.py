...
def stop_watch():
    global time
    time += (60/SPEED)
    millisecond = int(time % 60)
    second = int((time - millisecond)//60)
    minute = int((time - second)//60)
    arcade.draw_text(f"Time: {minute:02d}:{second:02d}:{millisecond:02d}", 75, SCREEN_HEIGHT - 50, arcade.color.BLUE,
